...
Options:\n
  --setup - Setup authentication for multiple gdindex domains.\n
...
_setup_authentication() {
    declare urls input _url _password list
    ! [[ -f "${AUTH_FILE}" ]] && : >| "${AUTH_FILE}"
    printf "%s\n\n" "Manual authentication setup. To change pass for existing url, just enter the same url and desired password."
    mapfile -t urls <<< "$(grep -v user_pass= < "${AUTH_FILE}")"
    if [[ -n ${urls[*]} ]]; then
    ...
    until [[ -n ${_exit} ]]; do
        ...
        if [[ ${input} = y ]]; then
            ...
            printf "%b\n" "${list:+${list}\n}${_url}\nuser_pass=${_username}:${_password}" >| "${AUTH_FILE}"
...
_check_url() {
    ...
    if headers="$(_fetch ${AUTH:+-u} ${AUTH} -I -X POST "${url}")"; then
        ...
        if ! [[ ${code} =~ ^(40.*|50.*)+$ ]]; then
            ...
            if [[ -n ${filename} ]]; then
                FILE_URL="${url}"
                FILE_SIZE="$(sed -n -e "s/content-length: //p" <<< "${headers//$'\r'/}")"
...
_download_file() {
    ...
    # shellcheck disable=SC2086
    [[ ${server_size} = zero ]] && : "$(curl -IsX POST ${AUTH:+-u} ${AUTH} "${file_url}" | sed -n -e "s/content-length: //p")" && server_size="${_//$'\r'/}"
...
_download_folder() {
    ...
    if [[ -n ${name} ]]; then
        ...
        if [[ -n ${PARSED_JSON} ]]; then
            ...
            mapfile -t files_list <<< "$(grep "/\"$" -v <<< "${PARSED_HTML}" | sed -e "s|^href=\"/|${BASE_URL}/|g" -e "s/\"$/__.__zero/g")" || :
            mapfile -t folders <<< "$(grep "/\"$" <<< "${PARSED_HTML}" | sed -e "s|^href=\"/|${BASE_URL}/|g" -e "s/\"$//g" -e 1d)" || :
        fi
...
_setup_arguments() {
    ...
    AUTH_FILE="${INFO_PATH}/auth.list" AUTH_LIST=""
    [[ -f ${AUTH_FILE} ]] && AUTH_LIST="$(< "${AUTH_FILE}")"
